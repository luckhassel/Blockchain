...
class Blockchain:
    ...
    def proof_of_work(self, previous_proof: int) -> int:
        ...
        while check_proof is False:
            #difficult level with 4 0's to the left
...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address, receiver='Guilherme',
                               amount=1)
...
def add_transaction():
    ...
    if not all(key in json for key in transaction_keys):
        return 'Key not found Error', 400
    index = blockchain.add_transaction(json['sender'], json['receiver'],
                                       json['amount'])
    response = {'message': f'Transaction added successfully to the block {index}!'}
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'Different chains: Chain substituted',
                    'new_chain': blockchain.chain}
    else:
        response = {'message': 'Equal chains: Chain not substituted',
